...
object TopStock {
  ...
  val year = 2017;
  val (topStock, topPrice) = symbols.map { ticker => (ticker, getYearEndClosingPrice(ticker, year)) }.maxBy { stockPrice => stockPrice._2 }
  ...
  def getYearEndClosingPrice(symbol: String, year: Int): BigDecimal = {
    val url = s"https://raw.githuhusercontent.com/ReactivePlatform/" +
